...
typedef Numeric (*UnaryArithmFunc) (Numeric num);
...
static JsonPathExecResult executeJsonPath(JsonPath *path, void *vars,
  ...
  Jsonb *js, bool throwErrors, JsonValueList *result, bool useTz);
...
static JsonPathExecResult executeItemUnwrapTargetArray(JsonPathExecContext *cxt,
  JsonPathItem *jsp, JsonbValue *jb, JsonValueList *found, bool unwrapElements);
...
static JsonPathExecResult executeAnyItem(JsonPathExecContext *cxt,
  JsonPathItem *jsp, JsonbContainer *jbc, JsonValueList *found, uint32 level,
  uint32 first, uint32 last, bool ignoreStructuralErrors, bool unwrapNext);
...
static JsonPathExecResult executeBinaryArithmExpr(JsonPathExecContext *cxt,
  JsonPathItem *jsp, JsonbValue *jb, BinaryArithmFunc func, JsonValueList *found);
static JsonPathExecResult executeUnaryArithmExpr(JsonPathExecContext *cxt,
  JsonPathItem *jsp, JsonbValue *jb, UnaryArithmFunc func, JsonValueList *found);
...
static JsonPathExecResult executeNumericItemMethod(JsonPathExecContext *cxt,
  JsonPathItem *jsp, JsonbValue *jb, bool unwrap, UnaryArithmFunc func,
  JsonValueList *found);
...
static void JsonItemFromDatum(Datum val, Oid typid, int32 typmod,
  JsonbValue *res);
static void JsonbValueInitNumeric(JsonbValue *jbv, Numeric num);
...
// static JsonTablePlanState *JsonTableInitPlan(JsonTableExecContext *cxt,
  // JsonTablePlan *plan, JsonTablePlanState *parentstate,
...
/**
 * @ingroup meos_json_base_jsonpath
...
#if MEOS
int
jsonb_path_exists(const Jsonb *jb, const JsonPath *jp, const Jsonb *vars,
  bool silent, bool tz)
{
...
#endif /* MEOS */
int
pg_jsonb_path_exists(const Jsonb *jb, const JsonPath *jp, const Jsonb *vars,
  bool silent, bool tz)
{
...
/**
 * @ingroup meos_json_base_jsonpath
 * @brief Return a JSON path expression result item for a JSONB value
...
#if MEOS
bool
jsonb_path_match(const Jsonb *jb, const JsonPath *jp, const Jsonb *vars,
  bool silent, bool tz)
{
...
#endif /* MEOS */
bool
pg_jsonb_path_match(const Jsonb *jb, const JsonPath *jp, const Jsonb *vars,
  bool silent, bool tz)
{
...
/**
 * @ingroup meos_json_base_jsonpath
 * @brief Execute a JSON path for a JSONB value and return the result as a
 * JSONB array
 * @param[in] jb JSONB value
 * @param[in] jp JSON path
 * @param[in] vars JSON variables, may be NULL
...
#if MEOS
Jsonb *
jsonb_path_query_array(const Jsonb *jb, const JsonPath *jp, const Jsonb *vars,
  bool silent, bool tz)
{
...
#endif /* MEOS */
Jsonb *
pg_jsonb_path_query_array(const Jsonb *jb, const JsonPath *jp,
...
/**
 * @ingroup meos_json_base_jsonpath
 * @brief Execute a JSON path for a JSONB value and return the first result
 * item. If there are no items, return NULL.
 * @param[in] jb JSONB value
 * @param[in] jp JSON path
 * @param[in] vars JSON variables, may be NULL
 * @param[in] silent When true, the following errors are suppressed: missing
...
#if MEOS
Jsonb *
jsonb_path_query_first(const Jsonb *jb, const JsonPath *jp, const Jsonb *vars,
  bool silent, bool tz)
{
...
#endif /* MEOS */
Jsonb *
pg_jsonb_path_query_first(const Jsonb *jb, const JsonPath *jp,
  const Jsonb *vars, bool silent, bool tz)
{
  JsonValueList found = {0};
  (void) executeJsonPath((JsonPath *) jp, (Jsonb *) vars,
...
/**
 * @ingroup meos_json_base_jsonpath
 * @brief Execute a JSON path for a JSONB value and return all result items as
 * a C array. If there are no items, return NULL.
 * @param[in] jb JSONB value
 * @param[in] jp JSON path
 * @param[in] vars JSON variables, may be NULL
 * @param[in] silent When true, the following errors are suppressed: missing
  ...
  bool silent, bool tz, int *count)
{
  ...
  assert(jb); assert(jp); assert(count);
  ...
  *count = JsonValueListLength(&found);
  ...
  JsonbValue *jbv;
  JsonValueListIterator it;
  JsonValueListInitIterator(&found, &it);
  int i = 0;
...
executeJsonPath(JsonPath *path, void *vars, JsonPathGetVarCallback getVar,
  JsonPathCountVarsCallback countVars, Jsonb *js, bool throwErrors,
  JsonValueList *result, bool useTz)
{
  ...
  if (!JsonbExtractScalar(&js->root, &jbv))
    JsonbInitBinary(&jbv, js);
  ...
  JsonbValue *jb, JsonValueList *found, bool unwrap)
{
  ...
  switch (jsp->type)
  {
    ...
    case jpiMinus:
      return executeUnaryArithmExpr(cxt, jsp, jb, pg_numeric_uminus, found);
    ...
    case jpiAnyKey:
      if (JsonbType(jb) == jbvObject)
      {
        ...
        return executeAnyItem(cxt, hasNext ? &elem : NULL,
    ...
    case jpiAbs:
      return executeNumericItemMethod(cxt, jsp, jb, unwrap, pg_numeric_abs,
        found);

    case jpiFloor:
      return executeNumericItemMethod(cxt, jsp, jb, unwrap, pg_numeric_floor,
        found);

    case jpiCeiling:
      return executeNumericItemMethod(cxt, jsp, jb, unwrap, pg_numeric_ceil,
        found);
    ...
    case jpiBigint:
      {
        ...
        {
          bool have_error;
          val = pg_numeric_int8_opt_error(jb->val.numeric, &have_error);
          if (have_error)
          {
            char *tmp = pg_numeric_out(jb->val.numeric);
            meos_error(ERROR, MEOS_ERR_SQL_JSON_ERROR,
        ...
        jb = &jbv;
        jb->type = jbvNumeric;
        jb->val.numeric = int64_to_numeric(val);
        res = executeNextItem(cxt, jsp, NULL, jb, found, true);
      }
      break;
    ...
    case jpiNumber:
      {
        ...
        if (jsp->type == jpiDecimal && jsp->content.args.left)
        {
          ...
          if (jsp->content.args.right)
          {
            ...
            scale = pg_numeric_int4_opt_error(jspGetNumeric(&elem),
              &have_error);
            if (have_error)
            {
              meos_error(ERROR, MEOS_ERR_SQL_JSON_ERROR,
          ...
          /* pg_numeric_typmodin() takes the precision and scale */
          int32 arrtypmod[2];
          arrtypmod[0] = precision;
          arrtypmod[1] = scale;
          int32 typmod = pg_numeric_typmodin(arrtypmod, 2);
          /* Convert numstr to Numeric with typmod */
          Assert(numstr != NULL);
          num = pg_numeric_in(numstr, typmod);
          if (! num)
          {
            meos_error(ERROR, MEOS_ERR_SQL_JSON_ERROR,
              "argument \"%s\" of jsonpath item method .%s() is invalid for type numeric",
    ...
    case jpiInteger:
      {
        ...
        jb = &jbv;
        jb->type = jbvNumeric;
        jb->val.numeric = int32_to_numeric(val);
        res = executeNextItem(cxt, jsp, NULL, jb, found, true);
      }
      break;
    ...
    case jpiStringFunc:
      {
        ...
        switch (JsonbType(jb))
        {
          ...
          case jbvBinary:
            meos_error(ERROR, MEOS_ERR_SQL_JSON_ERROR,
              "jsonpath item method .%s() can only be applied to a boolean, "
 ...
 */
static JsonPathExecResult
executeItemOptUnwrapResultNoThrow(JsonPathExecContext *cxt, JsonPathItem *jsp,
  JsonbValue *jb, bool unwrap, JsonValueList *found)
{
  JsonPathExecResult res;
    ...
    if (r == WJB_VALUE || r == WJB_ELEM)
    {
      ...
      if (level < last && v.type == jbvBinary)
      {
        res = executeAnyItem(cxt, jsp, v.val.binary.data, found,
           level + 1, first, last, ignoreStructuralErrors, unwrapNext);
        if (jperIsError(res))
          break;
...
executeBinaryArithmExpr(JsonPathExecContext *cxt, JsonPathItem *jsp,
  JsonbValue *jb, BinaryArithmFunc func, JsonValueList *found)
{
...
executeUnaryArithmExpr(JsonPathExecContext *cxt, JsonPathItem *jsp,
  JsonbValue *jb, UnaryArithmFunc func, JsonValueList *found)
{
  JsonPathExecResult jper;
  ...
  while ((val = JsonValueListNext(&seq, &it)))
  {
    ...
    if (func)
      val->val.numeric = func(val->val.numeric);
 ...
 */
static JsonPathBool
executeStartsWith(JsonPathItem *jsp UNUSED, JsonbValue *whole,
  JsonbValue *initial, void *param UNUSED)
{
  ...
  if (whole->val.string.len >= initial->val.string.len &&
      ! memcmp(whole->val.string.val, initial->val.string.val,
 ...
 */
static JsonPathBool
executeLikeRegex(JsonPathItem *jsp, JsonbValue *str, JsonbValue *rarg UNUSED,
  void *param)
{
  ...
  if (!cxt->regex)
  {
    cxt->regex = pg_cstring_to_text_with_len(jsp->content.like_regex.pattern,
  ...
  if (RE_compile_and_execute(cxt->regex, str->val.string.val,
      str->val.string.len, cxt->cflags, DEFAULT_COLLATION_OID, 0, NULL))
    return jpbTrue;
  return jpbFalse;
}
...
executeNumericItemMethod(JsonPathExecContext *cxt, JsonPathItem *jsp,
  JsonbValue *jb, bool unwrap, UnaryArithmFunc func, JsonValueList *found)
{
  JsonPathItem next;
  if (unwrap && JsonbType(jb) == jbvArray)
    return executeItemUnwrapTargetArray(cxt, jsp, jb, found, false);
  ...
  Numeric num = func(jb->val.numeric);
  ...
  JsonbValue *jb, JsonValueList *found)
{
  ...
  text *datetime = pg_cstring_to_text_with_len(jb->val.string.val,
    jb->val.string.len);
  ...
  else
  {
      ...
      jsp->content.arg)
    {
      ...
      time_precision = pg_numeric_int4_opt_error(jspGetNumeric(&elem),
        &have_error);
      if (have_error)
      {
        meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
    ...
    if (res == jperNotFound)
    {
      char *tmp = pg_text_to_cstring(datetime);
      if (jsp->type == jpiDatetime)
        meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
          "datetime format is not recognized: \"%s\"", tmp);
      else
        meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
          "%s format is not recognized: \"%s\"",
          jspOperationName(jsp->type), tmp);
      pfree(tmp);
      return jperError;
    }
  }
  ...
  /*
   * pg_parse_datetime() processes the entire input string per the template or
    ...
    case jpiDate:
      {
        ...
        switch (typid)
        {
          ...
          case TIMETZOID:
          {
            char *tmp = pg_text_to_cstring(datetime);
            meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
              "date format is not recognized: \"%s\"", tmp);
            pfree(tmp);
            return jperError;
          }
          case TIMESTAMPOID:
          ...
          case TIMESTAMPTZOID:
            ...
            value = timestamptz_to_date(value);
            break;
          default:
            meos_error(ERROR, MEOS_ERR_SQL_JSON_ERROR,
    ...
    case jpiTime:
      {
        ...
        switch (typid)
        {
          case DATEOID:
          {
            char *tmp = pg_text_to_cstring(datetime);
            meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
              "time format is not recognized: \"%s\"", tmp);
            pfree(tmp);
            return jperError;
          }
          ...
          case TIMETZOID:
            ...
            value = TimeADTGetDatum(timetz_to_time(DatumGetTimeTzADTP(value)));
            break;
          case TIMESTAMPOID:
            value = timestamp_to_time(value);
            break;
          case TIMESTAMPTZOID:
            checkTimezoneIsUsedForCast(cxt->useTz, "timestamptz", "time");
            value = timestamptz_to_time(value);
            break;
          default:
            meos_error(ERROR, MEOS_ERR_SQL_JSON_ERROR,
              "type with oid %u not supported", typid);
        ...
        if (time_precision != -1)
        {
          ...
          TimeADT result = DatumGetTimeADT(value);
    ...
    case jpiTimeTz:
      {
        ...
        switch (typid)
        {
          ...
          case TIMESTAMPOID:
          {
            char *tmp = pg_text_to_cstring(datetime);
            meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
              "time_tz format is not recognized: \"%s\"", tmp);
            pfree(tmp);
            return jperError;
          }
          case TIMEOID:
            checkTimezoneIsUsedForCast(cxt->useTz, "time", "timetz");
            value = PointerGetDatum(time_to_timetz(TimeADTGetDatum(value)));
            break;
          case TIMETZOID: /* Nothing to do for TIMETZ */
            break;
          case TIMESTAMPTZOID:
            value = PointerGetDatum(timestamptz_to_timetz(
        ...
        if (time_precision != -1)
        {
          ...
          TimeTzADT *result = DatumGetTimeTzADTP(value);
    ...
    case jpiTimestamp:
      {
        ...
        switch (typid)
        {
          ...
          case TIMETZOID:
          {
            char *tmp = pg_text_to_cstring(datetime);
            meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
              "timestamp format is not recognized: \"%s\"", tmp);
            pfree(tmp);
            return jperError;
          }
          ...
          case TIMESTAMPTZOID:
            ...
            value = timestamptz_to_timestamp(value);
            break;
          default:
            meos_error(ERROR, MEOS_ERR_SQL_JSON_ERROR,
              "type with oid %u not supported", typid);
        ...
        if (time_precision != -1)
        {
          ...
          Timestamp result = DatumGetTimestamp(value);
          bool success = AdjustTimestampForTypmod(&result, time_precision,
             NULL);
    ...
    case jpiTimestampTz:
      {
        ...
        switch (typid)
        {
          case DATEOID:
            ...
            /* Get the timezone value explicitly since JsonbValue keeps that
             * separate */
            ...
            value = date_to_timestamptz(value);
            break;
          case TIMEOID:
          case TIMETZOID:
          {
            char *tmp = pg_text_to_cstring(datetime);
            meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
              "timestamp_tz format is not recognized: \"%s\"", tmp);
            pfree(tmp);
          }
            break;
          case TIMESTAMPOID:
             ...
             * keeps that separate.
             */
            if (timestamp2tm(DatumGetTimestamp(value), NULL, &tm, &fsec,
                NULL, NULL) == 0)
              tz = DetermineTimeZoneOffset(&tm, session_timezone);
            value = timestamp_to_timestamptz(value);
            break;
        ...
        if (time_precision != -1)
        {
          ...
          Timestamp result = DatumGetTimestampTz(value);
          bool success = AdjustTimestampForTypmod(&result, time_precision,
            NULL);
          if (! success)  /* should not happen */
          {
...
JsonItemFromDatum(Datum val, Oid typid, int32 typmod, JsonbValue *res)
{
  switch (typid)
  {
    case BOOLOID:
      ...
      break;
    case NUMERICOID:
      JsonbValueInitNumeric(res, DatumGetNumeric(val));
      break;
    case INT2OID:
      JsonbValueInitNumeric(res, int16_to_numeric(DatumGetInt16(val)));
      break;
    case INT4OID:
      JsonbValueInitNumeric(res, int32_to_numeric(DatumGetInt32(val)));
      break;
    case INT8OID:
      JsonbValueInitNumeric(res, int64_to_numeric(DatumGetInt64(val)));
      break;
    case FLOAT4OID:
      JsonbValueInitNumeric(res, float4_to_numeric(DatumGetFloat4(val)));
      break;
    case FLOAT8OID:
      JsonbValueInitNumeric(res, float8_to_numeric(DatumGetFloat8(val)));
      break;
    case TEXTOID:
    case VARCHAROID:
      res->type = jbvString;
    ...
    default:
      meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
        "could not convert value of type %d to jsonpath", typid);
  }
}

/* Initialize numeric value from the given datum */
static void
JsonbValueInitNumeric(JsonbValue *jbv, Numeric num)
{
  jbv->type = jbvNumeric;
  jbv->val.numeric = num;
}
...
compareStrings(const char *mbstr1, int mblen1, const char *mbstr2, int mblen2)
{
  ...
  else
  {
    ...
    utf8len1 = (mbstr1 == utf8str1) ? mblen1 : (int) strlen(utf8str1);
    utf8len2 = (mbstr2 == utf8str2) ? mblen2 : (int) strlen(utf8str2);
...
compareItems(int32 op, JsonbValue *jb1, JsonbValue *jb2, bool useTz)
{
  ...
  switch (jb1->type)
  {
    ...
    case jbvBool:
      ...
      break;
    case jbvNumeric:
      cmp = pg_numeric_cmp(jb1->val.numeric, jb2->val.numeric);
      break;
    case jbvString:
      if (op == jpiEqual)
    ...
    case jbvDatetime:
      {
        ...
        cmp = compareDatetime(jb1->val.datetime.value, jb1->val.datetime.typid,
                jb2->val.datetime.value, jb2->val.datetime.typid, useTz,
                &cast_error);
        if (cast_error)
          return jpbUnknown;
      }
  ...
  int32 *index)
{
  ...
  Numeric numeric_index = pg_numeric_trunc(jbv->val.numeric, 0);
  *index = pg_numeric_int4_opt_error(numeric_index, &have_error);
 ...
 */
static JsonbValue *
JsonValueListNext(const JsonValueList *jvl UNUSED, JsonValueListIterator *it)
{
...
castTimeToTimeTz(Datum time, bool useTz)
{
  ...
  return TimeTzADTPGetDatum(time_to_timetz(time));
}
 ...
 */
static int
cmpDateToTimestamp(DateADT date1, Timestamp ts2, bool useTz UNUSED)
{
  return cmp_date_timestamp(date1, ts2);
}
...
cmpDateToTimestampTz(DateADT date1, TimestampTz tstz2, bool useTz)
{
  ...
  return cmp_date_timestamptz(date1, tstz2);
}
...
cmpTimestampToTimestampTz(Timestamp ts1, TimestampTz tstz2, bool useTz)
{
  ...
  return cmp_timestamp_timestamptz(ts1, tstz2);
}
 ...
 */
static int
compareDatetime(Datum val1, Oid typid1, Datum val2, Oid typid2, bool useTz,
  bool *cast_error)
{
  *cast_error = false;
  ...
  switch (typid1)
  {
    case DATEOID:
      ...
      {
        case DATEOID:
          return cmp_date_date(DatumGetDateADT(val1), DatumGetDateADT(val2));
    ...
    case TIMEOID:
      ...
      {
        case TIMEOID:
          return pg_time_cmp(DatumGetTimeADT(val1), DatumGetTimeADT(val2));

        case TIMETZOID:
          val1 = castTimeToTimeTz(val1, useTz);
          return pg_timetz_cmp(DatumGetTimeTzADTP(val1),
            DatumGetTimeTzADTP(val2));
    ...
    case TIMESTAMPOID:
      switch (typid2)
      {
        ...
        case TIMESTAMPOID:
          return cmp_timestamp_timestamp(DatumGetTimestamp(val1),
...
JsonPathExists(Datum jb, JsonPath *jp, bool *error, List *vars)
{
  JsonPathExecResult res = executeJsonPath(jp, vars, GetJsonPathVar,
    CountJsonPathVars, DatumGetJsonbP(jb), ! error, NULL, true);
  ...
  bool *error, List *vars, const char *column_name)
{
  ...
  JsonPathExecResult res = executeJsonPath(jp, vars, GetJsonPathVar,
    CountJsonPathVars, DatumGetJsonbP(jb), ! error, &found, true);
  Assert(error || ! jperIsError(res));
  if (error && jperIsError(res))
  {
    *error = true;
  ...
  {
    meos_error(ERROR, MEOS_ERR_SQL_JSON_ERROR,
      "unrecognized json wrapper %d", (int) wrapper);
    wrap = false;
  }
  ...
  const char *column_name)
{
  ...
  JsonValueList found = {0};
  JsonPathExecResult jper = executeJsonPath(jp, vars, GetJsonPathVar,
    CountJsonPathVars, DatumGetJsonbP(jb), ! error, &found, true);
  ...
  int count = JsonValueListLength(&found);
  *empty = (count == 0);
  if (*empty)
    return NULL;
...
JsonTableInitPlan(JsonTableExecContext *cxt, JsonTablePlan *plan,
  JsonTablePlanState *parentstate, List *args, MemoryContext mcxt)
{
...
JsonTableResetRowPattern(JsonTablePlanState *planstate, Datum item)
{
  ...
  res = executeJsonPath(planstate->path, planstate->args, GetJsonPathVar,
    CountJsonPathVars, js, scan->errorOnError, &planstate->found, true);
...
JsonTablePlanNextRow(JsonTablePlanState *planstate)
{
  if (IsA(planstate->plan, JsonTablePathScan))
    ...
    meos_error(ERROR, MEOS_ERR_SQL_JSON_ERROR,
      "invalid JsonTablePlan %d", (int) planstate->plan->type);
  Assert(false);
  /* Appease compiler */
  return false;
}
