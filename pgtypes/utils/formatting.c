...
// MEOS added is_digit and FromCharDateMode
/*  name, len, id, is_digit, FromCharDateMode      is in Index */ 
  {",", 1, NUM_COMMA, false, FROM_CHAR_DATE_NONE},  /* , */
  {".", 1, NUM_DEC, false, FROM_CHAR_DATE_NONE},   /* . */
  {"0", 1, NUM_0, false, FROM_CHAR_DATE_NONE},      /* 0 */
  {"9", 1, NUM_9, false, FROM_CHAR_DATE_NONE},      /* 9 */
  {"B", 1, NUM_B, false, FROM_CHAR_DATE_NONE},      /* B */
  {"C", 1, NUM_C, false, FROM_CHAR_DATE_NONE},      /* C */
  {"D", 1, NUM_D, false, FROM_CHAR_DATE_NONE},      /* D */
  {"EEEE", 4, NUM_E, false, FROM_CHAR_DATE_NONE},   /* E */
  {"FM", 2, NUM_FM, false, FROM_CHAR_DATE_NONE},    /* F */
  {"G", 1, NUM_G, false, FROM_CHAR_DATE_NONE},      /* G */
  {"L", 1, NUM_L, false, FROM_CHAR_DATE_NONE},      /* L */
  {"MI", 2, NUM_MI, false, FROM_CHAR_DATE_NONE},    /* M */
  {"PL", 2, NUM_PL, false, FROM_CHAR_DATE_NONE},    /* P */
  {"PR", 2, NUM_PR, false, FROM_CHAR_DATE_NONE},
  {"RN", 2, NUM_RN, false, FROM_CHAR_DATE_NONE},    /* R */
  {"SG", 2, NUM_SG, false, FROM_CHAR_DATE_NONE},    /* S */
  {"SP", 2, NUM_SP, false, FROM_CHAR_DATE_NONE},
  {"S", 1, NUM_S, false, FROM_CHAR_DATE_NONE},
  {"TH", 2, NUM_TH, false, FROM_CHAR_DATE_NONE},    /* T */
  {"V", 1, NUM_V, false, FROM_CHAR_DATE_NONE},      /* V */
  {"b", 1, NUM_B, false, FROM_CHAR_DATE_NONE},      /* b */
  {"c", 1, NUM_C, false, FROM_CHAR_DATE_NONE},      /* c */
  {"d", 1, NUM_D, false, FROM_CHAR_DATE_NONE},      /* d */
  {"eeee", 4, NUM_E, false, FROM_CHAR_DATE_NONE},   /* e */
  ...
  /* last */
  {NULL, 0, 0, false, FROM_CHAR_DATE_NONE}
};
...
from_char_seq_search(int *dest, const char **src, const char *const *array,
  char **localized_array, Oid collid, FormatNode *node, Node *escontext UNUSED)
{
  int len;
...
#undef DCH_to_char_fsec
      case DCH_SSSS:
        sprintf(s, "%lld",
            (long long) (tm->tm_hour * SECS_PER_HOUR +
               tm->tm_min * SECS_PER_MINUTE + tm->tm_sec));
        if (S_THth(n->suffix))
          str_numth(s, s, S_TH_TYPE(n->suffix));
        s += strlen(s);
        break;
      ...
      case DCH_MONTH:
        ...
        if (S_TM(n->suffix))
        {
          ...
          if (strlen(str) <= (size_t) (n->key->len + TM_SUFFIX_LEN) *
              DCH_MAX_ITEM_SIZ)
            strcpy(s, str);
          else
          {
 ...
 */
static void
DCH_from_char(FormatNode *node, const char *in, TmFromChar *out, Oid collid,
  bool std, Node *escontext)
{
  FormatNode *n;
  const char *s;
  int len, value;
  ...
  for (n = node, s = in; n->type != NODE_TYPE_END && *s != '\0'; n++)
  {
    ...
    switch (n->key->id)
    {
      ...
      case DCH_a_m:
      case DCH_p_m:
        if (! from_char_seq_search(&value, &s, ampm_strings_long, NULL,
            InvalidOid, n, escontext))
          return;
      ...
      case DCH_am:
      case DCH_pm:
        if (! from_char_seq_search(&value, &s, ampm_strings, NULL, InvalidOid,
            n, escontext))
          return;
        if (!from_char_set_int(&out->pm, value % 2, n, escontext))
          return;
      ...
      case DCH_US:    /* microsecond */
        len = from_char_parse_int_len(&out->us, &s, 
          n->key->id == DCH_US ? 6 : out->ff, n, escontext);
        if (len < 0)
          return;
      ...
      case DCH_TZ:
        {
          int tzlen = pg_DecodeTimezoneAbbrevPrefix(s, &out->gmtoffset,
              &out->tzp);
          if (tzlen > 0)
          {
            out->has_tz = true;
      ...
      case DCH_a_d:
      case DCH_b_c:
        if (!from_char_seq_search(&value, &s, adbc_strings_long, NULL,
            InvalidOid, n, escontext))
          return;
      ...
      case DCH_ad:
      case DCH_bc:
        if (! from_char_seq_search(&value, &s, adbc_strings, NULL, InvalidOid,
            n, escontext))
          return;
        if (! from_char_set_int(&out->bc, value % 2, n, escontext))
          return;
      ...
      case DCH_month:
        if (! from_char_seq_search(&value, &s, months_full,
            S_TM(n->suffix) ? localized_full_months : NULL, collid, n,
            escontext))
          return;
      ...
      case DCH_mon:
        if (! from_char_seq_search(&value, &s, months,
            S_TM(n->suffix) ? localized_abbrev_months : NULL, collid, n,
            escontext))
          return;
        if (! from_char_set_int(&out->mm, value + 1, n, escontext))
          return;
      ...
      case DCH_day:
        if (! from_char_seq_search(&value, &s, days,
            S_TM(n->suffix) ? localized_full_days : NULL, collid, n,
            escontext))
          return;
      ...
      case DCH_dy:
        if (! from_char_seq_search(&value, &s, days_short,
            S_TM(n->suffix) ? localized_abbrev_days : NULL, collid, n,
            escontext))
          return;
        if (!from_char_set_int(&out->d, value, n, escontext))
          return;
      ...
      case DCH_RM:
      case DCH_rm:
        if (!from_char_seq_search(&value, &s, rm_months_lower, NULL,
            InvalidOid, n, escontext))
          return;
 ...
 * @note Derived from PostgreSQL function @p timestamp_to_char()
 */
text *
timestamp_to_char(Timestamp dt, text *fmt)
{
  ...
  if (!(result = datetime_to_char_body(&tmtc, fmt, false, PG_GET_COLLATION())))
    return NULL;
  ...
  *fsec = 0;
  ...
  if (tmfc.ww)
  {
    ...
    else
    {
      ...
      {
        pg_DateTimeParseError(DTERR_FIELD_OVERFLOW, NULL, date_str, "timestamp",
          escontext);
        goto fail;
      }
    }
  ...
  if (tmfc.tzsign)
  {
    ...
    {
      pg_DateTimeParseError(DTERR_TZDISP_OVERFLOW, NULL, date_str, "timestamp",
        escontext);
      goto fail;
    }
 ...
 */
text *
float4_to_char(float4 value, text *fmt)
{
  NUMDesc    Num;
  FormatNode *format;
  text     *result;
 ...
 */
text *
float8_to_char(float8 value, text *fmt)
{
  NUMDesc    Num;
  FormatNode *format;
  text     *result;
