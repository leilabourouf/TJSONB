...
#ifdef __APPLE__
#ifndef strftime_l
#define strftime_l(buf,sz,fmt,tm,loc) strftime((buf),(sz),(fmt),(tm))
#endif
#endif
...
// MEOS
/**
 * @brief Global constant array containing collation provider strings
 * corresponding to the enumeration CollationProvider defined in file
 * `pg_collation_d.h`
 */
static const char *COLLATIONPROVIDER[] =
{
  [COLLPROV_DEFAULT] = "d",
  [COLLPROV_BUILTIN] = "b",
  [COLLPROV_ICU] = "i",
...
// {
  ...
  // collation_cache_start_iterate(CollationCache, &it);
  // while ((entry = collation_cache_iterate(CollationCache, &it)) != NULL)
  // {
    // if (entry->collid != NULL)
      // pfree(entry->collid); 
    // if (entry->locale != NULL)
      // pfree(entry->locale);
    // collation_cache_delete_item(CollationCache, entry);
  // }
  // collation_cache_destroy(tb);
// }
...
/*****************************************************************************
 * Definitions for reading the pg_collation.csv file
 *****************************************************************************/

/* Maximum length in characters of a header record in the input CSV file */
#define MAX_LENGTH_HEADER 1024
/* Location of the pg_collation.csv file */
char *PG_COLLATION_CSV = "/usr/local/share/pg_collation.csv";

/**
 * @brief Set the location of the PG_COLLATION_CSV files
 */
void
meos_set_pg_collation_csv(const char *path)
{
  PG_COLLATION_CSV = malloc(strlen(path) + 1);
  strcpy(PG_COLLATION_CSV, path);
}

/**
 * @brief Find collation information from the PG_COLLATION_CSV file
 * @return If found, returns a pointer to the function, otherwise NULL.
 */
pg_collation_record *
FindCollation(Oid collid)
{
  ...
  do
  {
    ...
    read = fscanf(file, "%d,%63[^,^\n],%c,%c,%d,%63[^,^\n],%63[^,^\n],%63[^,^\n],%63[^,^\n]\n",
      &rec.oid, rec.name, &rec.provider, &rec.isdeterministic,
      &rec.encoding, rec.ctype, rec.locale, rec.icurules, rec.version);
    ...
    if (read == 9 && collid == (uint32_t) rec.oid)
    {
      found = true;
      break;
    }
  ...
  pg_collation_record *result = palloc0(sizeof(pg_collation_record));
  return result;
}
 ...
 */
bool
check_locale_monetary(char **newval, void **extra UNUSED, GucSource source UNUSED)
{
  return check_locale(LC_MONETARY, *newval, NULL);
}

void
assign_locale_monetary(const char *newval UNUSED, void *extra UNUSED)
{
  CurrentLocaleConvValid = false;
}

bool
check_locale_numeric(char **newval, void **extra UNUSED, GucSource source UNUSED)
{
  return check_locale(LC_NUMERIC, *newval, NULL);
}

void
assign_locale_numeric(const char *newval UNUSED, void *extra UNUSED)
{
  CurrentLocaleConvValid = false;
}

bool
check_locale_time(char **newval, void **extra UNUSED, GucSource source UNUSED)
{
  return check_locale(LC_TIME, *newval, NULL);
}

void
assign_locale_time(const char *newval UNUSED, void *extra UNUSED)
{
 ...
 */
bool
check_locale_messages(char **newval, void **extra UNUSED, GucSource source)
{
  if (**newval == '\0')
  {
...
void
assign_locale_messages(const char *newval, void *extra UNUSED)
{
  /*
   * LC_MESSAGES category does not exist everywhere, but accept it anyway.
 ...
 */
static size_t
strftime_l_win32(char *dst, size_t dstlen, const char *format,
  const struct tm *tm, locale_t locale)
{
   ...
   */
  len = MultiByteToWideChar(CP_UTF8, 0, format, -1, wformat,
    lengthof(wformat));
  if (len == 0)
  {
    meos_error(ERROR, MEOS_ERR_INTERNAL_ERROR,
...
cache_single_string(char **dst, const char *src, int encoding)
{
  ...
  *dst = pg_any_to_server(src, strlen(src), encoding);
  return;
}
...
create_pg_locale(Oid collid)
{
  ...
  {
    /* shouldn't happen */
    PGLOCALE_SUPPORT_ERROR(rec->provider);
    return NULL;
  }
  ...
  if (strcmp(rec->version, "") != 0)
  {
    char *versionstr;
    if (rec->provider == COLLPROVIDER_LIBC)
      versionstr = rec->collate;
    else
      versionstr = rec->locale;
...
pg_newlocale_from_collation(Oid collid)
{
  ...
  {
    CollationCache = collation_cache_create(16, NULL);
  }
 ...
 */
char *
icu_language_tag(const char *loc_str UNUSED, int elevel UNUSED)
{
#ifdef USE_ICU
  UErrorCode  status;
  char     *langtag;
 ...
 */
void
icu_validate_locale(const char *loc_str UNUSED)
{
#ifdef USE_ICU
  UCollator  *collator;
  UErrorCode  status;
