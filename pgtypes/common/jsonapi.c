...
{
  void *pstack, *prediction, *fnames, *fnull;
...
#ifdef JSONAPI_USE_PQEXPBUFFER
  if (!lex->inc_state || !pstack || !prediction || !fnames || !fnull)
  {
    FREE(lex->inc_state);
    FREE(pstack);
    FREE(prediction);
...
inc_lex_level(JsonLexContext *lex)
{
  if (lex->incremental && (lex->lex_level + 1) >= lex->pstack->stack_size)
  {
    ...
    new_fnames = REALLOC(lex->pstack->fnames, new_stack_size * sizeof(char *));
#ifdef JSONAPI_USE_PQEXPBUFFER
    if (!new_fnames)
      return false;
#endif
 ...
 */
static inline JsonParseErrorType
json_lex_number(JsonLexContext *lex, const char *s, bool *num_err,
  size_t *total_len)
{
  bool error = false;
  int len = s - lex->input;
  ...
  if (len < (int) lex->input_length && *s == '0')
  {
    s++;
    len++;
  }
  else if (len < (int) lex->input_length && *s >= '1' && *s <= '9')
  {
    do
    {
      s++;
      len++;
    } while (len < (int) lex->input_length && *s >= '0' && *s <= '9');
  }
  else
    error = true;

  /* Part (3): parse optional decimal portion. */
  if (len < (int) lex->input_length && *s == '.')
  {
    s++;
    len++;
    if (len == (int) lex->input_length || *s < '0' || *s > '9')
      error = true;
    else
    {
      do
  ...
  if (len < (int) lex->input_length && (*s == 'e' || *s == 'E'))
  {
    s++;
    len++;
    if (len < (int) lex->input_length && (*s == '+' || *s == '-'))
    {
      s++;
      len++;
    }
   ...
   * purposes.
   */
  for (; len < (int) lex->input_length && JSON_ALPHANUMERIC_CHAR(*s);
      s++, len++)
    error = true;
