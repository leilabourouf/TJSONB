...
OpenTemporaryFile(bool interXact)
{
  ...
  if (file <= 0)
    file = OpenTemporaryFileInTablespace(MyDatabaseTableSpace ?
      MyDatabaseTableSpace : DEFAULTTABLESPACE_OID, true);
...
TempTablespacePath(char *path, Oid tablespace)
{
   ...
   */
  if (tablespace == InvalidOid || tablespace == DEFAULTTABLESPACE_OID ||
...
PathNameOpenTemporaryFile(const char *path, int mode)
{
  Assert(temporary_files_allowed);  /* check temp file access is up */
  ResourceOwnerEnlarge(CurrentResourceOwner);
  File file = PathNameOpenFile(path, mode | PG_BINARY);
...
OpenTransientFilePerm(const char *fileName, int fileFlags, mode_t fileMode)
{
  ...
  int fd = BasicOpenFilePerm(fileName, fileFlags, fileMode);
  if (fd >= 0)
  {
    AllocateDesc *desc = &allocatedDescs[numAllocatedDescs];
...
closeAllVfds(void)
{
  if (SizeVfdCache > 0)
  {
    ...
    for (Index i = 1; i < SizeVfdCache; i++)
    {
      if (!FileIsNotOpen(i))
        LruDelete(i);
    }
...
SetTempTablespaces(Oid *tableSpaces, int numSpaces)
{
   ...
   */
  if (numSpaces > 1)
    nextTempTableSpace = pg_prng_uint64_range(&pg_global_prng_state, 0, 
      numSpaces - 1);
  else
...
GetTempTablespaces(Oid *tableSpaces, int numSpaces)
{
  ...
  for (int i = 0; i < numTempTableSpaces && i < numSpaces; ++i)
...
{
  for (Index i = 0; i < numAllocatedDescs; i++)
  {
...
{
  cha temp_path[MAXPGPATH + sizeof(PG_TBLSPC_DIR) + 
    sizeof(TABLESPACE_VERSION_DIRECTORY) + sizeof(PG_TEMP_FILES_DIR)];
   ...
   */
  DIR *spc_dir = AllocateDir(PG_TBLSPC_DIR);
...
{
  DIR *temp_dir = AllocateDir(tmpdirname);
...
RemovePgTempRelationFilesInDbspace(const char *dbspacedirname)
{
  ...
  DIR *dbspace_dir = AllocateDir(dbspacedirname);
  struct dirent *de;
...
SyncDataDirectory(void)
{
  ...
  if (recovery_init_sync_method == DATA_DIR_SYNC_METHOD_SYNCFS)
  {
    ...
    DIR *dir = AllocateDir(PG_TBLSPC_DIR);
    struct dirent *de;
...
pre_sync_fname(const char *fname, bool isdir, int elevel)
{
  ...
  int fd = OpenTransientFile(fname, O_RDONLY | PG_BINARY);
  if (fd < 0)
  {
    if (errno == EACCES)
      return;
...
fsync_fname_ext(const char *fname, bool isdir, bool ignore_perm, int elevel)
{
  ...
  int fd = OpenTransientFile(fname, flags);
