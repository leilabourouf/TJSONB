 ...
 */
static pg_con_fn ToServerConvProc = NULL;
static pg_con_fn ToClientConvProc = NULL;
...
/**
 * @brief Set the active client encoding and set up the conversion-function
 * pointers
 * @return Return 0 if okay, -1 if not (bad encoding or can not support
 * conversion
 */
int
SetClientEncoding(int encoding)
{
  ...
  pg_con_fn to_server_proc = FindDefaultConversion(encoding,
    current_server_encoding);
  if (! to_server_proc)
    return -1;
  ToServerConvProc = to_server_proc;
  pg_con_fn to_client_proc = FindDefaultConversion(current_server_encoding,
    encoding);
  if (! to_client_proc)
    return -1;
  ToClientConvProc = to_client_proc;
 ...
 */
void
InitializeClientEncoding(int client_encoding)
{
  int current_server_encoding;

  if (SetClientEncoding(client_encoding) < 0)
  {
    /*
  ...
  if (current_server_encoding != PG_UTF8 &&
      current_server_encoding != PG_SQL_ASCII)
  {
    pg_con_fn utf8_to_server_proc = FindDefaultConversion(PG_UTF8,
      current_server_encoding);
    /* If there's no such conversion, just leave the pointer as NULL */
    if (utf8_to_server_proc)
      Utf8ToServerConvProc = utf8_to_server_proc;
  }
}
  ...
  int dest_encoding)
{
  ...
  unsigned char *result = (unsigned char *)
    palloc0((Size) len * MAX_CONVERSION_GROWTH + 1);

  (void) proc(src_encoding, dest_encoding, src, result, len, false);
 ...
 */
int
pg_do_encoding_conversion_buf(pg_con_fn proc, int src_encoding,
  int dest_encoding, unsigned char *src, int srclen, unsigned char *dest,
  int destlen, bool noError)
{
  /*
  ...
  return proc(src_encoding, dest_encoding, (unsigned char *) src,
    (unsigned char *) dest, srclen, noError);
}
...
bytea *
pg_convert_to(const bytea *string, const char *dest_encoding_name)
{
  const char *src_encoding_name = DatabaseEncoding->name;
  /*
   * pg_convert expects a bytea as its first argument. We're passing it a
   * text argument here, relying on the fact that they are both in fact
   * varlena types, and thus structurally identical.
   */
   return pg_convert(string, src_encoding_name, dest_encoding_name);
}
...
bytea *
pg_convert_from(const bytea *string, const char *src_encoding_name)
{
  const char *dest_encoding_name = DatabaseEncoding->name;
  /*
 ...
 */
bytea *
pg_convert(const bytea *string, const char *src_encoding_name,
  const char *dest_encoding_name)
{
  ...
  /* perform conversion */
  char *dest_str = (char *) pg_do_encoding_conversion((unsigned char *)
    unconstify(char *, src_str), len, src_encoding, dest_encoding);
 ...
 */
int
length_in_encoding(const bytea *string, char *src_encoding_name)
{
  int src_encoding = pg_char_to_encoding(src_encoding_name);
  if (src_encoding < 0)
  {
  ...
  int len = VARSIZE_ANY_EXHDR(string);
  const char *src_str = VARDATA_ANY(string);
  int result = pg_verify_mbstr_len(src_encoding, src_str, len, false);
  return result;
}
 ...
 */
int
pg_encoding_max_length_sql(int encoding)
{
...
pg_any_to_server(const char *s, int len, int encoding)
{
  ...
  if (encoding == (int) ClientEncoding->encoding)
    return perform_default_encoding_conversion(s, len, true);

  /* General case ... will not work outside transactions */
  return (char *) pg_do_encoding_conversion((unsigned char *)
    unconstify(char *, s), len, encoding, DatabaseEncoding->encoding);
}
...
pg_server_to_any(const char *s, int len, int encoding)
{
  ...
  /* General case ... will not work outside transactions */
  return (char *) pg_do_encoding_conversion((unsigned char *)
    unconstify(char *, s), len, DatabaseEncoding->encoding, encoding);
}
  ...
  bool is_client_to_server)
{
  ...
  result = (char *) palloc0((Size) len * MAX_CONVERSION_GROWTH + 1);

  pg_con_fn conv = FindDefaultConversion(src_encoding, dest_encoding);
  conv(src_encoding, dest_encoding, (unsigned char *) src,
    (unsigned char *) result, len, false);
...
pg_unicode_to_server(pg_wchar c, unsigned char *s)
{
  ...
  Utf8ToServerConvProc(PG_UTF8, server_encoding, (unsigned char *) c_as_utf8,
    (unsigned char *) s, c_as_utf8_len, false);
  return;
}
...
pg_unicode_to_server_noerror(pg_wchar c, unsigned char *s)
{
  ...
  converted_len = Utf8ToServerConvProc(PG_UTF8, server_encoding,
    (unsigned char *) c_as_utf8, (unsigned char *) s,
    c_as_utf8_len, true);
...
{
  return pg_wchar_table[DatabaseEncoding->encoding].
    mb2wchar_with_len((const unsigned char *) from, to, strlen(from));
}
...
{
  return pg_wchar_table[DatabaseEncoding->encoding].
    mb2wchar_with_len((const unsigned char *) from, to, len);
}

/* same, with any encoding */
int
pg_encoding_mb2wchar_with_len(int encoding, const char *from, pg_wchar *to,
  int len)
{
  return pg_wchar_table[encoding].
    mb2wchar_with_len((const unsigned char *) from, to, len);
}
...
{
  return pg_wchar_table[DatabaseEncoding->encoding].
    wchar2mb_with_len(from, (unsigned char *) to, pg_wchar_strlen(from));
}
...
{
  return pg_wchar_table[DatabaseEncoding->encoding].
    wchar2mb_with_len(from, (unsigned char *) to, len);
}

/* same, with any encoding */
int
pg_encoding_wchar2mb_with_len(int encoding, const pg_wchar *from,
  char *to, int len)
{
  return pg_wchar_table[encoding].
    wchar2mb_with_len(from, (unsigned char *) to, len);
...
pg_encoding_mbcliplen(int encoding, const char *mbstr, int len, int limit)
{
  ...
  while (len > 0 && *mbstr)
  {
    int l = (*mblen_fn) ((const unsigned char *) mbstr);
    if ((clen + l) > limit)
      break;
    clen += l;
    if (clen == limit)
...
{
  return pg_char_to_encoding(encoding_name);
}

const char *
PG_encoding_to_char(int32 encoding)
{
...
pg_verify_mbstr_len(int encoding, const char *mbstr, int len, bool noError)
{
  ...
  mbchar_verifier mbverifychar = pg_wchar_table[encoding].mbverifychar;
 ...
 */
void
check_encoding_conversion_args(int src_encoding, int dest_encoding, int len,
  ...
  const char *mbstr, int len)
{
   ...
   */
  int l = pg_encoding_mblen_or_incomplete(src_encoding, mbstr, len);
  int jlimit = Min(l, len);
  jlimit = Min(jlimit, 8);  /* prevent buffer overrun */
