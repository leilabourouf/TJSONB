...
create_pg_locale_icu(Oid collid)
{
  ...
  if (collid == DEFAULT_COLLATION_OID)
  {
    ...
    deterministic = true;
    iculocstr = database_locale;
    icurules = database_icurules;
  }
  else
  {
    pg_locale_t coll = FindCollation(collid);
    if (! coll)
    {
      // The error message has been ouput in function FindCollation
      // elog(ERROR, "cache lookup failed for collation %u", collid);
      return NULL;
    }
    deterministic = rec->isdeterministic;
    datum = SysCacheGetAttrNotNull(COLLOID, tp, Anum_pg_collation_colllocale);
    iculocstr = rec->locale;
    icurules = rec->icrules;
  }

  collator = make_icu_collator(iculocstr, coll.icurules);
  pg_locale_t result = palloc0(sizeof(struct pg_locale_struct));
  result->info.icu.locale = strdup(iculocstr);
...
pg_ucol_open(const char *loc_str)
{
  ...
  if (U_ICU_VERSION_MAJOR_NUM < 55)
  {
    ...
    {
      elog(ERROR, "could not get language from locale \"%s\": %s",
  ...
  if (U_FAILURE(status))
  {
    ...
    elog(ERROR, "could not open collator for locale \"%s\": %s",
  ...
  if (U_ICU_VERSION_MAJOR_NUM < 54)
  {
    ...
    {
      ucol_close(collator);
      elog(ERROR, "could not open collator for locale \"%s\": %s", orig_str,
        u_errorName(status));
    }
  }
  ...
  pg_locale_t locale)
{
  UChar *buff_uchar;
  UChar *buff_conv;
  int32_t len_uchar = icu_to_uchar(&buff_uchar, src, srclen);
  int32_t len_conv = icu_convert_case(u_strToLower, locale, &buff_conv, buff_uchar,
    len_uchar);
  size_t result_len = icu_from_uchar(dest, destsize, buff_conv, len_conv);
  pfree(buff_uchar);
  pfree(buff_conv);
  return result_len;
}
  ...
  pg_locale_t locale)
{
  ...
  UChar *buff_conv;
  int32_t len_uchar = icu_to_uchar(&buff_uchar, src, srclen);
  int32_t len_conv = icu_convert_case(u_strToTitle_default_BI, locale, &buff_conv,
  ...
  pg_locale_t locale)
{
  ...
  UChar *buff_conv;
  int32_t len_uchar = icu_to_uchar(&buff_uchar, src, srclen);
  int32_t len_conv = icu_convert_case(u_strToUpper, locale, &buff_conv,
    buff_uchar, len_uchar);
  size_t result_len = icu_from_uchar(dest, destsize, buff_conv, len_conv);
  pfree(buff_uchar);
  pfree(buff_conv);
  ...
  pg_locale_t locale)
{
  ...
  UChar *buff_conv;
  int32_t len_uchar = icu_to_uchar(&buff_uchar, src, srclen);
  int32_t len_conv = icu_convert_case(u_strFoldCase_default, locale,
...
#ifdef HAVE_UCOL_STRCOLLUTF8
int
strncoll_icu_utf8(const char *arg1, ssize_t len1, const char *arg2,
  ssize_t len2, pg_locale_t locale)
{
  ...
  int result = ucol_strcollUTF8(locale->info.icu.ucol, arg1, len1, arg2, len2,
    &status);
  if (U_FAILURE(status))
  {
    elog(ERROR, "collation failed: %s", u_errorName(status));
  }
  return result;
}
#endif
       ...
       pg_locale_t locale)
{
  ...
  UChar *uchar = (UChar *) buf;
  ulen = uchar_convert(icu_converter, uchar, ulen + 1, src, srclen);

  Size result_bsize = ucol_getSortKey(locale->info.icu.ucol, uchar, ulen,
  ...
  const char *src, ssize_t srclen, pg_locale_t locale)
{
  ...
  UErrorCode status = U_ZERO_ERROR;
  size_t result = ucol_nextSortKeyPart(locale->info.icu.ucol, &iter, state,
    (uint8_t *) dest, destsize, &status);
  if (U_FAILURE(status))
  {
    elog(ERROR, "sort key generation failed: %s", u_errorName(status));
  }
  return result;
}
...
{
  init_icu_converter();
  int32_t len_uchar = uchar_length(icu_converter, buff, nbytes);
  *buff_uchar = palloc((len_uchar + 1) * sizeof(**buff_uchar));
  len_uchar = uchar_convert(icu_converter, *buff_uchar, len_uchar + 1, buff,
    nbytes);
  return len_uchar;
}
...
icu_from_uchar(char *dest, size_t destsize, const UChar *buff_uchar, int32_t len_uchar)
{
  ...
  status = U_ZERO_ERROR;
  len_result = ucnv_fromUChars(icu_converter, NULL, 0, buff_uchar, len_uchar,
    &status);
  if (U_FAILURE(status) && status != U_BUFFER_OVERFLOW_ERROR)
  {
    elog(ERROR, "%s failed: %s", "ucnv_fromUChars", u_errorName(status));
  }
  ...
  UChar **buff_dest, UChar *buff_source, int32_t len_source)
{
  ...
  *buff_dest = palloc(len_dest * sizeof(**buff_dest));
  ...
  if (U_FAILURE(status))
  {
    elog(ERROR, "case conversion failed: %s", u_errorName(status));
  }
  return len_dest;
}

static int32_t
u_strToTitle_default_BI(UChar *dest, int32_t destCapacity, const UChar *src,
  int32_t srcLength, const char *locale, UErrorCode *pErrorCode)
{
  return u_strToTitle(dest, destCapacity, src, srcLength, NULL, locale,
    pErrorCode);
}

static int32_t
u_strFoldCase_default(UChar *dest, int32_t destCapacity, const UChar *src,
  int32_t srcLength, const char *locale, UErrorCode *pErrorCode)
{
       ...
       const char *arg2, ssize_t len2, pg_locale_t locale)
{
  ...
  int32_t ulen1 = uchar_length(icu_converter, arg1, len1);
  int32_t ulen2 = uchar_length(icu_converter, arg2, len2);
  int32_t bufsize1 = (ulen1 + 1) * sizeof(UChar);
  int32_t bufsize2 = (ulen2 + 1) * sizeof(UChar);

  if (bufsize1 + bufsize2 > TEXTBUFLEN)
    buf = palloc(bufsize1 + bufsize2);

  UChar uchar1 = (UChar *) buf;
  UChar uchar2 = (UChar *) (buf + bufsize1);

  ulen1 = uchar_convert(icu_converter, uchar1, ulen1 + 1, arg1, len1);
  ulen2 = uchar_convert(icu_converter, uchar2, ulen2 + 1, arg2, len2);

  int result = ucol_strcoll(locale->info.icu.ucol, uchar1, ulen1, uchar2, ulen2);
...
/* 'srclen' of -1 means the strings are NUL-terminated */
static size_t
strnxfrm_prefix_icu(char *dest, size_t destsize, const char *src,
  ssize_t srclen, pg_locale_t locale)
{
  char    sbuf[TEXTBUFLEN];
  char     *buf = sbuf;
  ...
  state[0] = state[1] = 0;  /* won't need that again */
  status = U_ZERO_ERROR;
  result_bsize = ucol_nextSortKeyPart(locale->info.icu.ucol, &iter, state,
    (uint8_t *) dest, destsize, &status);
  if (U_FAILURE(status))
  {
    elog(ERROR, "sort key generation failed: %s", u_errorName(status));
...
init_icu_converter(void)
{
  ...
  const char *icu_encoding_name =
    get_encoding_name_for_icu(GetDatabaseEncoding());
  if (!icu_encoding_name)
  {
    elog(ERROR, "encoding \"%s\" not supported by ICU",
  ...
  if (U_FAILURE(status))
  {
    elog(ERROR, "could not open ICU converter for encoding \"%s\": %s",
...
{
  UErrorCode status = U_ZERO_ERROR;
  int32_t ulen = ucnv_toUChars(converter, NULL, 0, str, len, &status);
  if (U_FAILURE(status) && status != U_BUFFER_OVERFLOW_ERROR)
  {
    elog(ERROR, "%s failed: %s", "ucnv_toUChars", u_errorName(status));
  }
  return ulen;
}
...
{
  UErrorCode status = U_ZERO_ERROR;
  int32_t ulen = ucnv_toUChars(converter, dest, destlen, src, srclen, &status);
  if (U_FAILURE(status))
  {
    elog(ERROR, "%s failed: %s", "ucnv_toUChars", u_errorName(status));
  }
