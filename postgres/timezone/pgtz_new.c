...
init_timezone_hashtable(void)
{
  ...
  timezone_cache = hash_create("Timezones", 4, &hash_ctl,
...
pg_tzset(const char *tzname)
{
  ...
  *p = '\0';
  ...
  tzp = (pg_tz_cache *) hash_search(timezone_cache, uppername, HASH_FIND,
    NULL);
  if (tzp)
  {
  ...
  tzp = (pg_tz_cache *) hash_search(timezone_cache, uppername, HASH_ENTER,
    NULL);
...
pg_tzset_offset(long gmtoffset)
{
  ...
  char offsetstr[64];
  char tzname[256]; // MEOS Previous value was 128
...
pg_tzenumerate_next(pg_tzenum *dir)
{
  while (dir->depth >= 0)
  {
    ...
    struct dirent *direntry = ReadDir(dir->dirdesc[dir->depth],
      dir->dirname[dir->depth]);
    if (! direntry)
    {
    ...
    snprintf(fullname, sizeof(fullname), "%s/%s", dir->dirname[dir->depth],
      direntry->d_name);
